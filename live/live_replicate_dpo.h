 ...
 */
typedef enum live_replicate_flags_t_
{
    LIVE_REPLICATE_FLAGS_NONE,
    LIVE_REPLICATE_FLAGS_HAS_LOCAL,
} __clib_packed live_replicate_flags_t;
...
typedef struct live_replicate_t_ {
   ...
   /**
     ...
     */
    live_replicate_flags_t rep_flags;
...
STATIC_ASSERT(sizeof(live_replicate_t) <= CLIB_CACHE_LINE_BYTES,
	      "A Live replicate object size exceeds one cacheline");
...
#endif /*__LIVE_REPLICATE_DPO_H__*/
