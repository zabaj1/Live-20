  ...
  vec_foreach_index (i, vec_policies)
  {
    ...
    vlib_cli_output (vm, "\tBehavior: %s",
		     (sr_policy->is_encap ? "Encapsulation" :
		      "SRH insertion"));
    if (sr_policy->type == SR_POLICY_TYPE_LIVE)
 	vlib_cli_output(vm, "\tType: Live-Live");
    else 
      {
    ...
    if (sr_policy->type == SR_POLICY_TYPE_LIVE)
    vlib_cli_output(vm, "\tLive-Live TLV: [8 Bytes]");
    vlib_cli_output(vm, "\ttype: 2, length [Bytes]: 7,  flow: --, sequence number: --");
    vlib_cli_output (vm, "-----------");
  }
  return 0;
}
